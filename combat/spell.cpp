#include "spell.h"
Spell::Spell(Noun source, Noun target, Verb action, std::vector<Word> adjectives)
{
    _source = source;
    _target = target;
    _action = action;
...
int Spell::cast(Mob* caster, BattleField* battleField)
{
    ...
    MapObject* target = _target.aquireTarget(caster, battleField);
    MapObject* source = _source.aquireTarget(caster, battleField);
